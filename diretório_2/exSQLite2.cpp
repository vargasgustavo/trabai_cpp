...
gcc -c sqlite3.c
compilar o codigo 
g++ exSQLIte2.c sqlite3.o -o exSQLite.exe
o arquivo sqlite.c e sqlite.h e sqlite.o devem estar no mesmo diretorio
*/
#include <iostream>
#include <iomanip>
#include "sqlite3.h"
#include "C:\Users\52318827\Desktop\trabai_cpp_c\diretório_2\myLib.cpp"
using namespace std;
...
void listar(sqlite3 *db){
    ...
    while (sqlite3_step(stmt) != SQLITE_DONE)
    {
        ...
        cout <<"|"<<setw(4)<<right<<id<<" | "<< setw(30) << left << nome << " | " 
        << setw(16) << cel << " | "<<setw(3) << idade << "   |"
        << endl;
    }
    cout << "+-----+--------------------------------+------------------+-------+" << endl;
}

bool consulta(sqlite3 *db, int id){
    sqlite3_stmt *stmt;
    char *erro;
    const unsigned char *nome;
    const unsigned char* cel;
    ...
    cout << "| ID  | NOME                           | CELULAR          | IDADE |" << endl;
    cout << "+-----+--------------------------------+------------------+-------+" << endl;
    cout <<"|" <<setw(4) << id << " | " << setw(30) << left << nome << " | "
         << setw(16) << cel << " | " << setw(3) << idade<<"   |"
         << endl;
    cout << "+-----+--------------------------------+------------------+-------+" << endl;
    } else {
...
void alterar (sqlite3 *db){
    ...
    char sql_c[200];
    acento;
    cout << "Qual o c�digo do registro a ser alterado: ";
    sacento;
    cin >> id;
    if (consulta(db, id)){	
        int op;
...
int main(int argc, char const *argv[])
{
    ...
    int rc = sqlite3_exec(db, "CREATE TABLE IF NOT EXISTS contatos (id integer primary key autoincrement, nome varchar(30), cel varchar(20), idade integer)", 
        NULL, NULL, &erro);
    if (rc != SQLITE_OK){
        cerr << "Erro " << erro << endl;
        return -1;
...
// g++ exSQLite2.cpp -l sqlite -o ./output/exSQLite2
